...
@Scope("session")
@Controller(value = "finAccountLogEditorController")
@RequestMapping(value = "/finAccountLog")
public class FinAccountLogEditorController extends SEEditorController {
	...
	public @ResponseBody String saveModuleService(@RequestBody String response) {
		...
		try {
			...
			FinAccountLogUIModel finAccountLogUIModel = (FinAccountLogUIModel) JSONObject
					.toBean(jsonObject, FinAccountLogUIModel.class, classMap);
			List<ServiceEntityNode> rawSeNodeList = finAccountManager
					.genSeNodeListInExtensionUnion(
							finAccountLogServiceUIModelExtension
									.genUIModelExtensionUnion().get(0),
							FinAccountLog.class, finAccountLogUIModel);
			finAccountManager.updateSeNodeListWrapper(rawSeNodeList,
					logonUser.getUuid(), organizationUUID);
			FinAccountLog finAccountLog = (FinAccountLog) finAccountManager
					.getEntityNodeByKey(finAccountLogUIModel.getUuid(),
							IServiceEntityNodeFieldConstant.UUID,
							FinAccountLog.NODENAME, logonUser.getClient(), null);
			finAccountLogUIModel = (FinAccountLogUIModel) finAccountManager
					.genUIModelFromUIModelExtension(FinAccountLogUIModel.class,
							finAccountLogServiceUIModelExtension
									.genUIModelExtensionUnion().get(0),
							finAccountLog, logonActionController.getLogonInfo(), null);
			return ServiceJSONParser.genDefOKJSONObject(finAccountLogUIModel);
	...
	public @ResponseBody String newModuleService(
		...
		try {
			...
			FinAccountLog finAccountLog = (FinAccountLog) finAccountManager
					.newEntityNode(finAccount, FinAccountLog.NODENAME);
			FinAccountLogUIModel finAccountLogUIModel = new FinAccountLogUIModel();
			finAccountManager.convFinAccountLogToUI(finAccountLog,
		...
		try {
			...
			FinAccountLog finAccountLog = (FinAccountLog) finAccountManager
					.getEntityNodeByKey(uuid,
							IServiceEntityNodeFieldConstant.UUID,
							FinAccountLog.NODENAME, logonUser.getClient(), null);
			String baseUUID = finAccountLog.getUuid();
			List<ServiceEntityNode> lockSEList = new ArrayList<ServiceEntityNode>();
			lockSEList.add(finAccountLog);
			...
			return lockObjectManager.genJSONLockCheckResult(lockResult,
					finAccountLog.getName(), finAccountLog.getId(), baseUUID);
		...
		try {
			...
			FinAccountLog finAccountLog = (FinAccountLog) finAccountManager
					.getEntityNodeByKey(uuid,
							IServiceEntityNodeFieldConstant.UUID,
							FinAccountLog.NODENAME, logonUser.getClient(), null);
			return ServiceJSONParser.genDefOKJSONObject(finAccountLog);
		} catch (AuthorizationException e) {
			return e.generateSimpleErrorJSON();
		} catch (LogonInfoException e) {
			return e.generateSimpleErrorJSON();
	...
	public @ResponseBody String loadModuleEditService(String uuid) {
		try {
			...
			FinAccountLogUIModel finAccountLogUIModel = (FinAccountLogUIModel) finAccountManager
					.genUIModelFromUIModelExtension(FinAccountLogUIModel.class,
							finAccountLogServiceUIModelExtension
									.genUIModelExtensionUnion().get(0),
							finAccountLog, logonActionController.getLogonInfo(),null);
