package com.company.IntelligentPlatform.platform.dto;

import java.util.Date;

import com.company.IntelligentPlatform.platform.controller.SEUIComModel;

/**
 * CorporateCustomer Excel Model
 **
 * @author
 * @date Sun Feb 14 11:35:39 CST 2016
 *
 *       This class is generated automatically by platform automation register
 *       tool
 */
public class CorporateCustomerExcelModel extends SEUIComModel {
	
	protected String uuid;

	protected String id;

	protected String name;

	protected String telephone;

	protected String fax;	

	protected String customerTypeValue;

	protected int customerType;

	protected String weiboID;

	protected String weiXinID;

	protected String faceBookID;
	
	protected String refSalesAreaName;

	protected String baseCityName;

	protected String postcode;
	
	protected String countryName;

	protected String stateName;

	protected String cityName;

	protected String townZone;

	protected String subArea;

	protected String streetName;
	
	protected String address;
	
	protected String email;

	protected String houseNumber;

	protected String retireReason;

	protected Date retireDate;

	protected String launchReason;

	protected Date launchDate;
	
	protected String refContactId;
	
	protected String refContactName;
	
	protected String refContactUUID;
	
	protected String refContactMobile;
	
	protected String refContactAddress;
	
	protected String refContactNote;
	
	protected String refContactRole;
	
	protected String refContactWeiXinId;
	
	protected String refContactEmail;

	public String getUuid() {
		return uuid;
	}

	public void setUuid(String uuid) {
		this.uuid = uuid;
	}

	public String getId() {
		return id;
	}

	public void setId(String id) {
		this.id = id;
	}

	public String getName() {
		return name;
	}

	public void setName(String name) {
		this.name = name;
	}

	public String getTelephone() {
		return telephone;
	}

	public void setTelephone(String telephone) {
		this.telephone = telephone;
	}

	public String getFax() {
		return fax;
	}

	public void setFax(String fax) {
		this.fax = fax;
	}

	public String getCustomerTypeValue() {
		return customerTypeValue;
	}

	public void setCustomerTypeValue(String customerTypeValue) {
		this.customerTypeValue = customerTypeValue;
	}

	public int getCustomerType() {
		return customerType;
	}

	public void setCustomerType(int customerType) {
		this.customerType = customerType;
	}

	public String getWeiboID() {
		return weiboID;
	}

	public void setWeiboID(String weiboID) {
		this.weiboID = weiboID;
	}

	public String getWeiXinID() {
		return weiXinID;
	}

	public String getCountryName() {
		return countryName;
	}

	public void setCountryName(String countryName) {
		this.countryName = countryName;
	}

	public String getStateName() {
		return stateName;
	}

	public void setStateName(String stateName) {
		this.stateName = stateName;
	}

	public void setWeiXinID(String weiXinID) {
		this.weiXinID = weiXinID;
	}

	public String getFaceBookID() {
		return faceBookID;
	}

	public void setFaceBookID(String faceBookID) {
		this.faceBookID = faceBookID;
	}

	public String getBaseCityName() {
		return baseCityName;
	}

	public void setBaseCityName(String baseCityName) {
		this.baseCityName = baseCityName;
	}

	public String getPostcode() {
		return postcode;
	}

	public void setPostcode(String postcode) {
		this.postcode = postcode;
	}

	public String getCityName() {
		return cityName;
	}

	public void setCityName(String cityName) {
		this.cityName = cityName;
	}

	public String getTownZone() {
		return townZone;
	}

	public void setTownZone(String townZone) {
		this.townZone = townZone;
	}

	public String getSubArea() {
		return subArea;
	}

	public void setSubArea(String subArea) {
		this.subArea = subArea;
	}

	public String getStreetName() {
		return streetName;
	}

	public void setStreetName(String streetName) {
		this.streetName = streetName;
	}

	public String getHouseNumber() {
		return houseNumber;
	}

	public void setHouseNumber(String houseNumber) {
		this.houseNumber = houseNumber;
	}

	public String getAddress() {
		return address;
	}

	public void setAddress(String address) {
		this.address = address;
	}

	public String getEmail() {
		return email;
	}

	public void setEmail(String email) {
		this.email = email;
	}

	public String getRetireReason() {
		return retireReason;
	}

	public void setRetireReason(String retireReason) {
		this.retireReason = retireReason;
	}

	public Date getRetireDate() {
		return retireDate;
	}

	public void setRetireDate(Date retireDate) {
		this.retireDate = retireDate;
	}

	public String getLaunchReason() {
		return launchReason;
	}

	public void setLaunchReason(String launchReason) {
		this.launchReason = launchReason;
	}

	public Date getLaunchDate() {
		return launchDate;
	}

	public void setLaunchDate(Date launchDate) {
		this.launchDate = launchDate;
	}

	public String getRefSalesAreaName() {
		return refSalesAreaName;
	}

	public void setRefSalesAreaName(String refSalesAreaName) {
		this.refSalesAreaName = refSalesAreaName;
	}

	public String getRefContactId() {
		return refContactId;
	}

	public void setRefContactId(String refContactId) {
		this.refContactId = refContactId;
	}

	public String getRefContactName() {
		return refContactName;
	}

	public void setRefContactName(String refContactName) {
		this.refContactName = refContactName;
	}

	public String getRefContactUUID() {
		return refContactUUID;
	}

	public void setRefContactUUID(String refContactUUID) {
		this.refContactUUID = refContactUUID;
	}

	public String getRefContactMobile() {
		return refContactMobile;
	}

	public void setRefContactMobile(String refContactMobile) {
		this.refContactMobile = refContactMobile;
	}

	public String getRefContactAddress() {
		return refContactAddress;
	}

	public void setRefContactAddress(String refContactAddress) {
		this.refContactAddress = refContactAddress;
	}

	public String getRefContactNote() {
		return refContactNote;
	}

	public void setRefContactNote(String refContactNote) {
		this.refContactNote = refContactNote;
	}

	public String getRefContactRole() {
		return refContactRole;
	}

	public void setRefContactRole(String refContactRole) {
		this.refContactRole = refContactRole;
	}

	public String getRefContactWeiXinId() {
		return refContactWeiXinId;
	}

	public void setRefContactWeiXinId(String refContactWeiXinId) {
		this.refContactWeiXinId = refContactWeiXinId;
	}

	public String getRefContactEmail() {
		return refContactEmail;
	}

	public void setRefContactEmail(String refContactEmail) {
		this.refContactEmail = refContactEmail;
	}

}
