 ...
 *
 * <p>When {@code nodeName} equals {@link ServiceEntityNode#NODENAME_ROOT} ("ROOT"),
 * the JPA entity name cannot be used as-is.  The {@code seConfigureProxy} is used
 * to look up the concrete {@code nodeType} and return its simple class name instead.</p>
 */
public class JpaServiceEntityDAO implements ServiceEntityDAO {
    ...
    private final ServiceEntityConfigureProxy seConfigureProxy;

    @SuppressWarnings("rawtypes")
    public JpaServiceEntityDAO(EntityManager entityManager, JpaRepository<?, String> repository,
            ServiceEntityConfigureProxy seConfigureProxy) {
    ...
    /**
     * Resolve the JPQL entity name from a nodeName.
     * If nodeName is "ROOT", look up the concrete Java class via the configure proxy.
     * For all other nodeNames, the nodeName is already the Java class name.
     */
    private String resolveEntityName(String nodeName) {
        if (!ServiceEntityNode.NODENAME_ROOT.equals(nodeName)) {
            return nodeName;
        }
        if (seConfigureProxy == null) {
            return nodeName;
        }
        try {
            ServiceEntityConfigureMap configMap = seConfigureProxy.getConfigureByNodeName(nodeName);
            if (configMap == null || configMap.getNodeType() == null) {
                return nodeName;
            }
            return configMap.getNodeType().getSimpleName();
        } catch (Exception e) {
            logger.warn("resolveEntityName: could not resolve ROOT node name, using as-is: {}", e.getMessage());
            return nodeName;
        }
    }
    ...
    public List<String> getEntityFieldListByKeyList(List<ServiceBasicKeyStructure> keyList, String nodeName,
        ...
        try {
            String entityName = resolveEntityName(nodeName);
            String alias = alias(entityName);
    ...
    private List<Object> appendWhereClause(StringBuilder jpql, String alias,
        ...
        if (keyList != null && !keyList.isEmpty()) {
            ...
            for (int i = 0; i < keyList.size(); i++) {
                ...
                List<?> multiList = key.getMultipleValueList();
                if (multiList != null && !multiList.isEmpty()) {
                    appendInCondition(jpql, alias, key.getKeyName(), multiList, params);
                } else {
    ...
    /**
     * Appends an IN clause for a list of values using positional parameters.
     */
    private void appendInCondition(StringBuilder jpql, String alias, String fieldName,
            List<?> values, List<Object> params) {
        jpql.append(alias).append(".").append(fieldName).append(" in (");
        for (int i = 0; i < values.size(); i++) {
            if (i > 0) {
                jpql.append(", ");
            }
            params.add(values.get(i));
            jpql.append("?").append(params.size());
        }
        jpql.append(")");
    }
    ...
    private List<ServiceEntityNode> queryByKey(Object keyValue, String keyName, String nodeName,
        ...
        try {
            String entityName = resolveEntityName(nodeName);
            String alias = alias(entityName);
            StringBuilder jpql = new StringBuilder("from ").append(entityName).append(" ").append(alias);
            List<Object> params = new ArrayList<>();
            if (keyName != null) {
                jpql.append(" where ");
                appendCondition(jpql, alias, keyName, keyValue, preciseMatchFlag, params);
                ...
                jpql.append(" where ").append(alias).append(".client = ?1");
                params.add(client);
            }
