...
import com.company.IntelligentPlatform.platform.service.SearchAuthorizationContext;
import com.company.IntelligentPlatform.platform.service.SpringContextBeanService;
import com.company.IntelligentPlatform.platform.service.SysAODetermineProxy;
import com.company.IntelligentPlatform.platform.service.SysAODeterminer;
import com.company.IntelligentPlatform.platform.service.SystemAuthorizationObjectManager;
...
public class AuthorizationManager {
    ...
    protected Logger logger = LoggerFactory.getLogger(AuthorizationManager.class);
    ...
    public boolean checkAuthorization(LogonUser logonUser, int aoType,
        ...
        logger.debug("checkAuthorization: user={} targetAO={} action={} roles={}", logonUser.getId(), targetAOID, actionCode, roleList.size());
        logger.debug("checkAuthorization: rawAuthorizationGroupList size={}", rawAuthorizationGroupList == null ? 0 : rawAuthorizationGroupList.size());
        for (Role role : roleList) {
            List<ServiceEntityNode> roleAOList = roleManager
                    .getEntityNodeListByKey(role.getUuid(),
                            ...
                            null);
            logger.debug("checkAuthorization: checking role={} roleAOList={}", role.getId(), roleAOList == null ? 0 : roleAOList.size());
        ...
        logger.debug("checkAuthorization: no access for user={} targetAO={}", logonUser.getId(), targetAOID);
        return false;
    }
    ...
    public Map<AuthorizationObject, List<ActionCode>> calculateSubSystemAuthorMap(RoleAuthorization roleAuthorization
        ...
        for (ServiceEntityNode serviceEntityNode : roleSubAuthorizationList) {
            ...
            if (authorizationObject == null) {
                authorizationObject = (AuthorizationObject) systemAuthorizationObjectManager.getEntityNodeByKey(
                        roleSubAuthorization.getRefUUID(),
                        IServiceEntityNodeFieldConstant.UUID,
                        SystemAuthorizationObject.NODENAME,
    ...
    public boolean getRoleAccess(LogonUser logonUser, String roleUUID,
        ...
        if (roleAGAOMap.size() <= 0) {
            logger.debug("getRoleAccess: roleAGAOMap empty for role={}", roleUUID);
            return false;
        }
        logger.debug("getRoleAccess: roleAGAOMap size={} keys={}", roleAGAOMap.size(), roleAGAOMap.keySet());
        /*
        ...
        logger.debug("getRoleAccess: selectAGAOMap size={} keys={}", selectAGAOMap.size(), selectAGAOMap.keySet());
        /**
        ...
        while (iterator2.hasNext()) {
            ...
            AuthorizationGroup authorizationGroup = (AuthorizationGroup) ServiceCollectionsHelper
                    .filterSENodeOnline(agUUID, rawAuthorizationGroupList);
            boolean procResult = this.processAuthorizationGroup(logonUser, authorizationGroup,
                    roleAOList, aoMap, targetAOID, actionCode, target,
                    rawActionCodeList, homeOrganization,
                    organizationList);
            logger.debug("getRoleAccess: processAuthorizationGroup agUUID={} result={}", agUUID, procResult);
            if (procResult) {
                result = true;
                break;
            } else {
        ...
        logger.debug("getRoleAccess: final result={}", result);
        return result;
            ...
            while (iterator.hasNext()) {
                ...
                List<ActionCode> actionCodeList = actionCodeManager
                        .filterActionCodeByActionCodeArray(roleAuthorization,
                                rawActionCodeList);
                if (actionCodeList == null || actionCodeList.size() == 0) {
                    logger.debug("processAuthorizationGroup: no actionCodes for roleAOUUID={} ao={}", roleAOUUID, ao == null ? null : ao.getId());
                    continue;
                }
                boolean dispatchResult = getAOAccessDispatcher(logonUser, ao, targetAOID,
                        actionCode, target, homeOrganization, organizationList,
                        actionCodeList);
                logger.debug("processAuthorizationGroup: ao={} target={} action={} -> {}", ao == null ? null : ao.getId(), targetAOID, actionCode, dispatchResult);
                if (!dispatchResult) {
                    continue;
                } else {
                    groupResult = true;
