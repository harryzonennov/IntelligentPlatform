...
import com.company.IntelligentPlatform.platform.service.ServiceEntityConfigureProxyService;
import com.company.IntelligentPlatform.platform.service.ServiceModelExtensionHelper;
import com.company.IntelligentPlatform.platform.service.ServiceModelExtensionHelper.ExtensionUnionResponse;
import com.company.IntelligentPlatform.platform.service.ServiceExtensionManager;
...
public class ServiceModuleProxy {
	...
	private void loadFillSubNode(ServiceModule serviceModuleInstance,
		...
		String effectiveNodeName = ServiceEntityNode.NODENAME_ROOT.equals(subListFieldConfigure.nodeName())
				? subNodeInstId
				: subListFieldConfigure.nodeName();
		List<ServiceEntityNode> subSENodeList = this.loadSubNodeListWrapper(
				parentNode, serviceEntityManager,
				effectiveNodeName, subExtensionUnion);
		if (listFlag) {
		...
		if (listFlag) {
			...
			if (ServiceEntityFieldsHelper.checkSuperClassExtends(
				...
				if (!ServiceCollectionsHelper.checkNullList(subSENodeList)) {
					@SuppressWarnings("rawtypes")
					List subServiceModuleList = new ArrayList();
					ServiceEntityManager effectiveManager = subExtensionUnion.getServiceEntityManager() != null
							? subExtensionUnion.getServiceEntityManager()
							: serviceEntityManager;
					for (ServiceEntityNode tmpSENode : subSENodeList) {
						ServiceModule tmpServiceModule = loadServiceModule(
								ServiceEntityFieldsHelper
										.getListSubType(subField),
								effectiveManager, tmpSENode, subExtension,
								subNodeInstId);
						subServiceModuleList.add(tmpServiceModule);
					}
					if (!ServiceCollectionsHelper
	...
	private List<ServiceEntityNode> loadSubNodeListWrapper(
		...
		if (subExtensionUnion.getToParentModelNodeMapConfigure() != null) {
			...
			ServiceEntityManager effectiveManager = subExtensionUnion.getServiceEntityManager() != null
					? subExtensionUnion.getServiceEntityManager()
					: serviceEntityManager;
			List<ServiceEntityNode> subSENodeList = effectiveManager
