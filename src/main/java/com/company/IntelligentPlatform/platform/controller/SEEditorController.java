...
import com.company.IntelligentPlatform.platform.service.ServiceJSONDataHelper;
...
import com.company.IntelligentPlatform.platform.model.IServiceJSONBasicErrorCode;
...
public class SEEditorController extends SEBasicController {
	...
	private static final Logger logger = LoggerFactory.getLogger(SEEditorController.class);
	...
	public ServiceEntityNode getServiceEntityNodeFromBuffer(String nodeName,
			String baseUUID) {
		if (seBindList == null) {
			logger.debug("getServiceEntityNodeFromBuffer: empty list");
			return null;
		} else {
			logger.debug("getServiceEntityNodeFromBuffer: list size={}", seBindList.size());
		}
	...
	public List<ServiceEntityNode> getSEListNodeFromBuffer(String nodeName,
		...
		if (seBindList == null) {
			logger.debug("getSEListNodeFromBuffer: empty list for Node:[{}] baseUUID:[{}]", nodeName, baseUUID);
		} else {
			logger.debug("getSEListNodeFromBuffer: list size={}", seBindList.size());
		}
	...
	public void save(String baseUUID, ServiceEntityManager seManager, LogonUser logonUser, Organization organization)
		...
		if (seBindListBack == null || seBindListBack.size() == 0) {
			...
			try {
				...
				logger.warn("LockObjectFailureException during lock — should not occur here", e);
			}
		}
