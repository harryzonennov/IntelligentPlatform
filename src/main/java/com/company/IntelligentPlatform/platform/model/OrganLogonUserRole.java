package com.company.IntelligentPlatform.platform.model;

import com.company.IntelligentPlatform.platform.model.*;
import com.company.IntelligentPlatform.platform.model.*;
import com.company.IntelligentPlatform.platform.model.*;
import com.company.IntelligentPlatform.platform.model.*;
import com.company.IntelligentPlatform.platform.model.*;
import jakarta.persistence.Entity;
import jakarta.persistence.Table;

/**
 * Record speci
 * 
 * @author ZhangHang
 * @date Nov 25, 2012
 * 
 */
@Entity
@Table(name = "OrganLogonUserRole", catalog = "platform")
public class OrganLogonUserRole extends ReferenceNode {

	public static final String NODENAME = IServiceModelConstants.OrganLogonUserRole;

	public static final String SENAME = Organization.SENAME;

	public int USERROLE_ORG_MAN = 1;

	public int USERROLE_ORG_VICEMAN = 2;

	public int USERROLE_ORG_FINRES = 3;

	protected int logonUserRole;

	public OrganLogonUserRole() {
		super.nodeName = NODENAME;
		super.serviceEntityName = SENAME;
	}

	public int getLogonUserRole() {
		return logonUserRole;
	}

	public void setLogonUserRole(int logonUserRole) {
		this.logonUserRole = logonUserRole;
	}
}
