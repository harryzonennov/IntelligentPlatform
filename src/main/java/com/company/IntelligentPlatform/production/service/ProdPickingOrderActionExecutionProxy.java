...
@Service
public class ProdPickingOrderActionExecutionProxy
    ...
    protected Logger logger = LoggerFactory.getLogger(ProdPickingOrderActionExecutionProxy.class);
    ...
    public DocumentContentSpecifier<ProdPickingOrderServiceModel, ProdPickingOrder, ProdPickingRefMaterialItem> getDocumentContentSpecifier() {
        return prodPickingOrderSpecifier;
    }
    ...
    public void executeActionCore(ProdPickingOrderServiceModel prodPickingOrderServiceModel, int docActionCode,
        ...
        super.defExecuteActionCore(prodPickingOrderServiceModel, docActionCode,
                (prodPickingOrder, serialLogonInfo1) -> prodPickingOrder, (prodPickingRefMaterialItem, itemSelectionExecutionContext) -> {
                    if (docActionCode == ProdPickingOrderActionNode.DOC_ACTION_APPROVE) {
                        super.checkUpdateItemStatus(prodPickingRefMaterialItem, docActionCode, serialLogonInfo, false,
                                (pickItem, itemSelectionExecutionContext2) -> {
                                    ProdPickingRefOrderItemServiceModel prodPickingRefOrderItemServiceModel =
                                            ServiceCollectionsHelper.filterOnline(
                                                    prodPickingOrderServiceModel.getProdPickingRefOrderItemList(),
                                                    refOrderItemServiceModel -> pickItem.getParentNodeUUID()
                                                            .equals(refOrderItemServiceModel.getProdPickingRefOrderItem()));
                                    LogonInfo logonInfo = logonInfoManager.genLogonInfo(serialLogonInfo,false);
                                    prodPickingOrderManager.approvePickingRefMaterialItem(pickItem,
                                            prodPickingRefOrderItemServiceModel, logonInfo);
                                    return true;
                                });
                    }
                    if (docActionCode == ProdPickingOrderActionNode.DOC_ACTION_DELIVERY_DONE) {
                        try {
                            prodPickingOrderManager.setFinishPickingRefMaterialItem(prodPickingRefMaterialItem,
                                    prodPickingOrderServiceModel.getProdPickingOrder(), true,
