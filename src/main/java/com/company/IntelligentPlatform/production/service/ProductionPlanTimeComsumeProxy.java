 ...
 */
@Service
public class ProductionPlanTimeComsumeProxy {
	...
	public void calculateProdBOMItemLeadTime(
		...
		for (ServiceEntityNode seNode : footerBOMList) {
			...
			leadTime = ServiceEntityDoubleHelper.trancateDoubleScale4(leadTime);
	 ...
	 * Implementation logic to calculate lead time by process route as well as
	 * the implementation to calculate by material own information
	 * 
	 * @param leadTimeCalMode
	 * @param refRouteOrderUUID
	 * @param ratioToStandardUnit: Ratio about current unit to standard unit
	 ...
	 * @throws MaterialException
	 */
	public void processSetProductionPlanLeadTime(
		...
		MaterialStockKeepUnit materialStockKeepUnit = materialStockKeepUnitManager
				.getMaterialSKUWrapper(
						productionPlan.getRefMaterialSKUUUID(),
						productionPlan.getClient(), rawMaterialList);
		double ratioToStandardUnit = getRatioToStandardUnit(productionPlan,
		...
		double selfLeadTime = calculateSelfLeadTime(
				...
				ratioToStandardUnit, materialStockKeepUnit,
				productionPlan.getAmount(), productionPlan.getRefUnitUUID());
		selfLeadTime = ServiceEntityDoubleHelper.trancateDoubleScale4(selfLeadTime);
		...
		productionPlan.setSelfLeadTime(ServiceEntityDoubleHelper.trancateDoubleScale2(selfLeadTime));
		productionPlan.setComLeadTime(ServiceEntityDoubleHelper.trancateDoubleScale2(selfLeadTime + maxSubLeadTime));		
	...
	protected double getRatioToStandardUnit(
		...
		storageCoreUnitBOM.setAmount(1);
	 ...
	 * @throws ServiceEntityConfigureException
	 */
	protected double getRatioToStandardUnit(ProductionPlan productionPlan,
			MaterialStockKeepUnit materialStockKeepUnit)
			throws MaterialException, ServiceEntityConfigureException {
		StorageCoreUnit storageCoreUnitBOM = new StorageCoreUnit();
		storageCoreUnitBOM.setAmount(productionPlan.getAmount());
		storageCoreUnitBOM.setRefMaterialSKUUUID(productionPlan
				.getRefMaterialSKUUUID());
		storageCoreUnitBOM.setRefUnitUUID(productionPlan.getRefUnitUUID());
		StorageCoreUnit storageCoreMaterial = new StorageCoreUnit();
		storageCoreMaterial.setAmount(1);
		storageCoreMaterial.setRefMaterialSKUUUID(materialStockKeepUnit
				.getUuid());
