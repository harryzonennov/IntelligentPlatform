...
import com.company.IntelligentPlatform.production.dto.ProdPickingRefMaterialItemServiceUIModel;
import com.company.IntelligentPlatform.production.dto.ProdPickingRefOrderItemServiceUIModel;
import com.company.IntelligentPlatform.production.dto.ProdPickingRefOrderItemServiceUIModelExtension;
...
import com.company.IntelligentPlatform.production.service.ProdPickingRefOrderItemServiceModel;
import com.company.IntelligentPlatform.production.service.ProdPickingRefOrderltemManager;
...
import com.company.IntelligentPlatform.platform.service.ServiceUIModuleProxyException;
...
@Scope("session")
@Controller(value = "prodReturnRefOrderItemEditorController")
@RequestMapping(value = "/prodReturnRefOrderItem")
public class ProdReturnRefOrderItemEditorController extends SEEditorController {
	...
	public @ResponseBody String getPageHeaderModelList(
		...
		try {
			...
			List<PageHeaderModel> pageHeaderModelList = prodPickingRefOrderltemManager
					.getPageHeaderModelList(prodPickingRefOrderItem,
							logonUser.getClient());
		...
		try {
			...
			ProdPickingRefOrderItemServiceModel prodPickingRefOrderItemServiceModel = (ProdPickingRefOrderItemServiceModel) prodPickingOrderManager
					.genServiceModuleFromServiceUIModel(
							ProdPickingRefOrderItemServiceModel.class,
							ProdPickingRefOrderItemServiceUIModel.class,
							prodPickingRefOrderItemServiceUIModel,
	...
	public @ResponseBody String newModuleService(
		...
		try {
			...
			prodPickingOrderManager.convProdPickingRefOrderItemToUI(
					prodPickingRefOrderItem, prodPickingRefOrderItemUIModel);
			return ServiceJSONParser
					.genDefOKJSONObject(prodPickingRefOrderItemUIModel);
		} catch (LogonInfoException | AuthorizationException e) {
			return e.generateSimpleErrorJSON();
		} catch (ServiceEntityConfigureException e) {
			return ServiceJSONParser.generateSimpleErrorJSON(e.getMessage());
	...
	public @ResponseBody String preLock(String uuid) {
		try {
			...
			return lockObjectManager.genJSONLockCheckResult(lockResult,
					prodPickingRefOrderItem.getName(),
					prodPickingRefOrderItem.getId(), baseUUID);
		...
		try {
			...
			boolean checkAuthor = serviceBasicUtilityController
					.checkTargetDataAccess(logonInfo.getLogonUser(),
							prodPickingRefOrderItem, ISystemActionCode.ACID_EDIT,
			...
			return ServiceJSONParser
					.genDefOKJSONObject(prodPickingRefOrderItem);
	...
	public @ResponseBody String loadFromProdOrderService(String baseUUID) {
		try {
			serviceBasicUtilityController.preCheckResourceAccessCore(
					AOID_RESOURCE, ISystemActionCode.ACID_EDIT);
			LogonUser logonUser = logonActionController.getLogonUser();
			...
			List<ServiceEntityNode> prodPickingRefOrderItemList = prodPickingOrderManager
					.getEntityNodeListByKey(baseUUID, "refProdOrderUUID",
							ProdPickingRefOrderItem.NODENAME,
							logonUser.getClient(), null);
	...
	private ProdPickingRefOrderItemServiceUIModel refreshLoadServiceUIModel(
		...
		ProdPickingRefOrderItemServiceModel prodPickingRefOrderItemServiceModel = (ProdPickingRefOrderItemServiceModel) prodPickingOrderManager
				.loadServiceModule(ProdPickingRefOrderItemServiceModel.class,
						prodPickingRefOrderItem);
		// set the gross order cost
		prodPickingOrderManager
				.setGrossOrderCost(prodPickingRefOrderItemServiceModel);
		ProdPickingRefOrderItemServiceUIModel prodPickingRefOrderItemServiceUIModel = (ProdPickingRefOrderItemServiceUIModel) prodPickingOrderManager
				.genServiceUIModuleFromServiceModel(
						ProdPickingRefOrderItemServiceUIModel.class,
						ProdPickingRefOrderItemServiceModel.class,
		...
		try {
			...
			lockObjectManager.lockServiceEntityList(lockSEList, logonUser,
					logonActionController.getOrganizationByUser(logonUser
							.getUuid()));			
			ProdPickingRefOrderItemServiceUIModel prodPickingRefOrderItemServiceUIModel = refreshLoadServiceUIModel(
