...
@Scope("session")
@Controller(value = "calendarTempWorkScheduleEditorController")
@RequestMapping(value = "/calendarTempWorkSchedule")
public class CalendarTempWorkScheduleEditorController extends
		SEEditorController {
	...
	public @ResponseBody String saveModuleService(@RequestBody String response) {
		...
		try {
			...
			CalendarTempWorkScheduleUIModel calendarTempWorkScheduleUIModel = (CalendarTempWorkScheduleUIModel) JSONObject
					.toBean(jsonObject, CalendarTempWorkScheduleUIModel.class,
							classMap);
			List<ServiceEntityNode> rawSeNodeList = calendarTemplateManager
					.genSeNodeListInExtensionUnion(
							calendarTempWorkScheduleServiceUIModelExtension
			...
			CalendarTempWorkSchedule calendarTempWorkSchedule = (CalendarTempWorkSchedule) calendarTemplateManager
					.getEntityNodeByKey(
							calendarTempWorkScheduleUIModel.getUuid(),
							IServiceEntityNodeFieldConstant.UUID,
							CalendarTempWorkSchedule.NODENAME,
							logonUser.getClient(), null);
			calendarTempWorkScheduleUIModel = (CalendarTempWorkScheduleUIModel) calendarTemplateManager
					.genUIModelFromUIModelExtension(
							CalendarTempWorkScheduleUIModel.class,
							calendarTempWorkScheduleServiceUIModelExtension
									.genUIModelExtensionUnion().get(0),
							calendarTempWorkSchedule, logonActionController.getLogonInfo(), null);
			return ServiceJSONParser
					.genDefOKJSONObject(calendarTempWorkScheduleUIModel);
		...
		try {
			...
			CalendarTempWorkSchedule calendarTempWorkSchedule = (CalendarTempWorkSchedule) calendarTemplateManager
					.newEntityNode(calendarTemplate,
							CalendarTempWorkSchedule.NODENAME);
			CalendarTempWorkScheduleUIModel calendarTempWorkScheduleUIModel = new CalendarTempWorkScheduleUIModel();
			calendarTemplateManager.convCalendarTempWorkScheduleToUI(
					calendarTempWorkSchedule, calendarTempWorkScheduleUIModel);
			return ServiceJSONParser
					.genDefOKJSONObject(calendarTempWorkScheduleUIModel);
		} catch (LogonInfoException e) {
			return e.generateSimpleErrorJSON();
		...
		try {
			...
			CalendarTempWorkSchedule calendarTempWorkSchedule = (CalendarTempWorkSchedule) calendarTemplateManager
					.getEntityNodeByKey(uuid,
							...
							CalendarTempWorkSchedule.NODENAME,
							logonUser.getClient(), null);
			String baseUUID = calendarTempWorkSchedule.getUuid();
			List<ServiceEntityNode> lockSEList = new ArrayList<>();
			lockSEList.add(calendarTempWorkSchedule);
			...
			return lockObjectManager.genJSONLockCheckResult(lockResult,
					calendarTempWorkSchedule.getName(),
					calendarTempWorkSchedule.getId(), baseUUID);
		...
		try {
			...
			CalendarTempWorkSchedule calendarTempWorkSchedule = (CalendarTempWorkSchedule) calendarTemplateManager
					.getEntityNodeByKey(uuid,
							...
							logonUser.getClient(), null);
			return ServiceJSONParser
					.genDefOKJSONObject(calendarTempWorkSchedule);
		} catch (AuthorizationException e) {
			return e.generateSimpleErrorJSON();
		} catch (LogonInfoException e) {
			return e.generateSimpleErrorJSON();
	...
	public @ResponseBody String loadModuleEditService(String uuid) {
		try {
			...
			CalendarTempWorkScheduleUIModel calendarTempWorkScheduleUIModel = (CalendarTempWorkScheduleUIModel) calendarTemplateManager
					.genUIModelFromUIModelExtension(
							CalendarTempWorkScheduleUIModel.class,
							calendarTempWorkScheduleServiceUIModelExtension
									.genUIModelExtensionUnion().get(0),
